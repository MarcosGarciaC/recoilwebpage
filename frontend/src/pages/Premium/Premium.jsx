...
const FeatureCard = ({ number, color, title, description, icon, wide }) => (
    <div className={`feature-card ${wide ? 'feature-card--wide' : ''} reveal reveal-up`}>
        <div className="feature-card__icon" style={{ color }}>{icon}</div>
...
const Perk = ({ text }) => (
    <li className="perk">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#9CFF93" className="perk-icon" viewBox="0 0 16 16">
...
const Premium = () => {
    ...
    // ── NUEVO: detectar si el usuario ya tiene suscripción activa ─────────────
    const [isPremium, setIsPremium] = useState(false);

    useEffect(() => {
        try {
            const currentUser = JSON.parse(localStorage.getItem("currentUser")) || {};
            setIsPremium(currentUser.premium === true);
        } catch (e) {
            console.error("Error leyendo currentUser:", e);
        }
    }, []);

    // Si ya tiene premium, el botón redirige a /subscription en lugar de /payment
    ...
    useEffect(() => {
        ...
        const animateCounter = () => {
            ...
            const counter = setInterval(() => {
                ...
                if (users >= usersTarget) users = usersTarget
                if (satisfactionValue >= satisfactionTarget) satisfactionValue = satisfactionTarget
                if (users >= 1000) setActiveUsers(`${Math.floor(users / 1000)}K+`)
                else setActiveUsers(Math.floor(users).toString())
                setSatisfaction(`${Math.floor(satisfactionValue)}%`)
                if (users === usersTarget && satisfactionValue === satisfactionTarget) clearInterval(counter)
            }, increment)
        }
        const observer = new IntersectionObserver(
            entries => {
    ...
    return (
        <>
            ...
            <section className="premium-hero">
                ...
                >
                    <div className={`volume-bar-wrap ${showVolumeBar || isDragging ? 'volume-bar-wrap--visible' : ''}`}>
                        ...
                        >
                            <div className="volume-bar-fill" style={{ height: `${volume * 100}%` }} />
                            <div className="volume-bar-thumb" style={{ bottom: `calc(${volume * 100}% - 8px)` }} />
                        </div>
                                ...
                                <div className="pricing-card__inner">
                                    ...
                                    <div className="pricing-card__top">
                                        <div>
                                            ...
                                            <p className="pricing-card__uplink">UNLIMITED UPLINK</p>
                                        </div>
                                    ...
                                    <ul className="perks-list">
                                        {perks.map((p, i) => <Perk key={i} text={p} />)}
                                    </ul>

                                    {/*
                                        ── NUEVO: botón con dos estados según isPremium ──
                                        - Si ya tiene sub: texto informativo, redirige a /subscription
                                        - Si no tiene: flujo normal hacia /payment
                                    */}
                                    <button
                                        className={`cta-btn ${btnHovered ? 'cta-btn--hovered' : ''} ${btnClicked ? 'cta-btn--clicked' : ''} ${isPremium ? 'cta-btn--already-premium' : ''}`}
                                        onMouseEnter={() => { setBtnHovered(true); if (!isPremium) playHover() }}
                                        onMouseLeave={() => setBtnHovered(false)}
                                        onClick={handleBtnClick}
                                        title={isPremium ? "Ya tienes una suscripción activa" : ""}
                                    >
                                        <span className="cta-btn__glow" />
                                        <span className="cta-btn__scanline" />
                                        <span className="cta-btn__text">
                                            {isPremium
                                                ? '✓ YA CUENTAS CON SUSCRIPCIÓN'
                                                : btnClicked
                                                    ? '// INICIANDO PROTOCOLO...'
                                    ...
                                    <p className="cta-sub">
                                        {isPremium
                                            ? 'ADMINISTRA TU PLAN DESDE TU PERFIL'
            ...
            <section className="mid-operative reveal reveal-up" ref={statsRef}>
                ...
                <p className="pricing-eyebrow">LIVE DATA FEED / 2024</p>
                <h2 className="mid-operative__title">PLAN OPERATIVO</h2>
                <p className="pricing-label">UNLIMITED UPLINK</p>
                ...
                <div className="mid-operative-grid">
                    ...
                    <div className="featured-review">
                        ...
                        <p className="featured-review__footer">MIEMBRO DESDE 2023</p>
                    </div>
                    ...
                    <div className="stats-column">
                        <div className="stat-box">
                            ...
                            <div>
                                <p className="stat-box__label">OPERADORES ACTIVOS</p>
                                <h3 className="stat-box__number">{activeUsers}</h3>
                                <span className="stat-box__sub">EN TODO EL SISTEMA RECOIL</span>
                            </div>
                        </div>
                        <div className="stat-box">
                            <div className="stat-box__icon">
                            ...
                            <div>
                                <p className="stat-box__label">SATISFACCIÓN</p>
                                <h3 className="stat-box__number">{satisfaction}</h3>
                                <span className="stat-box__sub">DE OPERADORES APROBADOS</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section className="premium-testimonials reveal reveal-up">
                ...
                <div className="testimonials-grid">
                    {[
                        { handle: '@Manolo_the_critic', text: 'La insignia verificada cambió cómo la comunidad recibe mis reseñas. Autoridad real.', score: '9.8', avatar: user2 },
                        { handle: '@Lure', text: 'El acceso temprano a features es increíble. Ya probé tres módulos antes del lanzamiento.', score: '9.5', avatar: user3 },
                        { handle: '@AntonioGamer', text: 'Sin ads, sin ruido. Solo el contenido que importa. RECOIL Premium vale cada centavo.', score: '10.0', avatar: user4 }
                    ].map((t, i) => (
